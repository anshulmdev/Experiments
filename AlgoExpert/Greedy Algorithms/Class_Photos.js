...
function classPhotos(redShirtHeights, blueShirtHeights) {
  ...
  redShirtHeights.sort((a,b) => b - a);
	...
	// RED: [1, 3, 4, 5, 8, 9]
	// BLUE: [1, 2, 4, 5, 6, 9]
	const tallColor = redShirtHeights[0] > blueShirtHeights[0] ? 'red' : 'blue'
	for (let i=0; i < arrayLength; i++) {
		if (tallColor === 'red' && redShirtHeights[i] <= blueShirtHeights[i]) return false
		if (tallColor === 'blue' && redShirtHeights[i] >= blueShirtHeights[i]) return false
	}
